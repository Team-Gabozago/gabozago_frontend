...
const FeedComponent = ({ post }: { post: Feed }) => {
    const { author, comments, content, likes, title, updated_at, images } =
        post;
    ...
    return (
        <div className="w-80 flex justify-center items-center mb-2 rounded bg-white">
            {images && (
                <img
                    src={images[0]}
                    className="w-36 h-full bg-lightGray"
                    alt={`${title}'s feedImage`}
                />
            )}
            <div className="p-4 flex-1">
                <p className="text-xs text-label mb-1.5">
                    {calculateDate(updated_at)}
                </p>
